...
.guideline-items-toolbar { display:flex; align-items:center; justify-content:space-between; gap:8px; flex-wrap:wrap; }
.guideline-items-count { font-size:12px; color:#64748b; font-weight:600; }
.guideline-items-hint { font-size:11px; color:#94a3b8; }
.guideline-items-search {
  flex:1; min-width:140px; height:34px; box-sizing:border-box;
  border:1px solid #cbd5e1; border-radius:8px; padding:0 10px;
  font-size:12px; background:#fff; color:#0f172a;
}
.guideline-items-search:focus { outline:none; border-color:#2563eb; box-shadow:0 0 0 2px rgba(37,99,235,.12); }
.guideline-add-btn {
...
.guideline-items-footer {
  display:flex; flex-direction:column; gap:6px; padding-top:8px;
  border-top:1px dashed #e2e8f0;
}
.guideline-add-btn-block {
  width:100%; padding:12px 14px; font-size:14px; border-radius:10px;
}
...
`;
  let style = document.getElementById('guideline-items-editor-style');
  if (!style) {
...
function mountGuidelineItemsEditor(container, options = {}) {
  ...
  const emptyText = options.emptyText || '지침이 없습니다. + 지침 추가로 넣어 주세요.';
  ...
  toolbar.innerHTML = `
    ...
    <input type="search" class="guideline-items-search" placeholder="지침 검색" aria-label="지침 검색" />
  ...
  const searchEl = toolbar.querySelector('.guideline-items-search');
  ...
  function categoryLabel(categoryId) {
    return GUIDELINE_CATEGORIES.find((cat) => cat.id === (categoryId || 'other'))?.label || '';
  }
  ...
  function render() {
    ...
    const filledCount = items.filter((i) => String(i.text || '').trim()).length;
    const filteredFilled = filtered.filter((i) => String(i.text || '').trim()).length;
    countEl.textContent = searchQuery.trim()
      ? `${filteredFilled}/${filledCount}개 표시`
      : `${filledCount}개 지침`;
    ...
    if (!filtered.length) {
      listEl.innerHTML = `<div class="guideline-empty">「${escapeGuidelineHtml(searchQuery.trim())}」 검색 결과가 없습니다.</div>`;
      return;
    }

    const drafts = filtered.filter((item) => !String(item.text || '').trim());
    const filled = filtered.filter((item) => String(item.text || '').trim());
    const groups = GUIDELINE_CATEGORIES.map((cat) => ({
      ...cat,
      items: filled.filter((item) => (item.category || 'other') === cat.id),
    })).filter((group) => group.items.length);

    const draftHtml = drafts.length
      ? `
      <section class="guideline-group" data-category="draft">
        <div class="guideline-group-title">새 지침</div>
        <div class="guideline-group-items">
          ${drafts.map((item) => renderItemCard(item)).join('')}
        </div>
      </section>`
      : '';
    ...
    listEl.innerHTML = draftHtml + groupsHtml;
    ...
    listEl.querySelectorAll('.guideline-item-text').forEach((textarea) => {
      ...
      textarea.addEventListener('blur', (event) => {
        const id = textarea.dataset.id;
        const article = textarea.closest('.guideline-item');
        const related = event.relatedTarget;
        // 같은 카드의 분류/삭제로 포커스 이동 시에는 빈 칸을 지우지 않음
        if (article && related && article.contains(related)) return;

        window.setTimeout(() => {
          const target = items.find((item) => item.id === id);
          if (!target) return;
          const active = document.activeElement;
          if (article && active && article.contains(active)) return;
  ...
  function addGuidelineItem() {
    ...
    const card = listEl.querySelector(`.guideline-item[data-id="${id}"]`);
  ...
  toolbar.querySelector('[data-action="add"]').addEventListener('click', addGuidelineItem);
