...
async function postAiApi(path, body, signal) {
  ...
  if (!response.ok || data.ok === false) {
    ...
    const err = new Error(
      response.status === 401
        ? '로그인이 만료되었습니다. [계정]에서 다시 로그인해 주세요.'
...
async function fetchAuthApi(path, options = {}) {
  ...
  if (!response.ok || data.ok === false) {
    ...
    const err = new Error(
      response.status === 401 && options.auth !== false
        ? '로그인이 만료되었습니다. 다시 로그인해 주세요.'
        : data.error || `서버 오류 (${response.status})`
    );
    if (response.status === 401 && options.auth !== false) err.code = 'AUTH_EXPIRED';
    throw err;
  }
...
async function refreshAccountUsage(options = {}) {
  ...
  accountSyncPromise = (async () => {
    try {
      ...
      if (err?.code === 'AUTH_EXPIRED' || /로그인/.test(String(err?.message || ''))) {
        await clearAuthSession();
      }
      throw err;
    }
...
async function openBillingPage(planId = 'standard') {
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime?.id) {
    params.set('extId', chrome.runtime.id);
  }
...
function formatSubscriptionSummary(subscription) {
  ...
  if (subscription.active) {
    ...
    const pendingText = subscription.pendingPlanId
      ? ` · 만료일부터 ${subscription.pendingPlanName || subscription.pendingPlanId}로 전환 예약`
      : '';
    const cardText = subscription.cardLabel ? ` · 카드 ${subscription.cardLabel}` : '';
    if (subscription.planId !== 'pro') {
      return `구독 중 · ${subscription.planName || subscription.planId} · 만료 ${subscription.expiresAt || '-'}${renewText}${graceText}${pendingText}${cardText} · [플랜 변경] 가능`;
    }
    return `구독 중 · ${subscription.planName || subscription.planId} · 만료 ${subscription.expiresAt || '-'}${renewText}${graceText}${pendingText}${cardText}`;
  }
  return '무료 체험 이용 중 · [구독하기]에서 플랜 선택';
}
...
function isTrialUsage(usage) {
  return !!usage?.trial || usage?.planId === 'none' || usage?.planName === '무료 체험';
}

function replyLimitReachedText(usage) {
  if (isTrialUsage(usage)) {
    return `무료 체험 답글 한도(${usage?.replyLimit || 20}건)를 모두 사용했습니다. [계정]에서 구독해 주세요.`;
  }
  return `이번 달 답글 생성 한도(${usage?.replyLimit || 0}건)를 모두 사용했습니다.`;
}

function getReplyRemaining(usage) {
  if (!usage || usage.locked) return null;
...
function validateReplyGenerationCount(usage, requestedCount) {
  ...
  if (remaining <= 0) {
    return {
      ok: false,
      usage,
      message: formatUsageLimitMessage(replyLimitReachedText(usage), 0),
      remaining: 0,
    };
  }

  if (count > remaining) {
    const scope = isTrialUsage(usage) ? '무료 체험' : '이번 달';
    return {
      ok: false,
      usage,
      message:
        `선택 ${count}건은 ${scope} 남은 한도(${remaining}건)를 초과합니다.\n\n` +
        `${remaining}건 이하로 선택하거나 [계정] 탭에서 구독해 주세요.`,
      remaining,
    };
  }
...
function buildReplyUsageNotice(usage, selectedCount, options = {}) {
  ...
  if (remaining <= 0) {
    return {
      level: 'error',
      text: `${summary} · ${replyLimitReachedText(usage)}`,
    };
  }
...
function formatUsageSummary(usage) {
  ...
  const period = usage.periodLabel || (isTrialUsage(usage) ? '체험' : usage.period || '');
  return (
    `${usage.planName || usage.planId || '플랜'} · ` +
    `답글 ${usage.replyUsed}/${usage.replyLimit} · ` +
    `말투 ${usage.toneUsed}/${usage.toneLimit} (${period})`
  );
}
