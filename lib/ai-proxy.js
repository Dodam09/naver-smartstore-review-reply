...
function formatSubscriptionSummary(subscription) {
  ...
  if (subscription.active) {
    if (subscription.cancelled || subscription.status === 'cancelled') {
      return `구독 취소됨 · ${subscription.planName || subscription.planId} · ${subscription.expiresAt || '-'}까지 이용 · [다시 구독하기]로 연장`;
    }
    if (subscription.planId !== 'pro') {
      return `구독 중 · ${subscription.planName || subscription.planId} · 만료 ${subscription.expiresAt || '-'} · [플랜 업그레이드] 가능`;
    }
