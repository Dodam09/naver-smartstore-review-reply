...
function formatSubscriptionSummary(subscription) {
  ...
  if (subscription.active) {
    ...
    if (subscription.planId !== 'pro') {
      return `구독 중 · ${subscription.planName || subscription.planId} · 만료 ${subscription.expiresAt || '-'}${renewText}${graceText}${pendingText} · [플랜 변경] 가능`;
    }
    return `구독 중 · ${subscription.planName || subscription.planId} · 만료 ${subscription.expiresAt || '-'}${renewText}${graceText}${pendingText}`;
  }
