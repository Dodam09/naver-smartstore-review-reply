...
function getConfiguredStyleSnapshot(settings = {}, context = 'review') {
  ...
  const presetId = settings[presetIdKey] || CUSTOM_PRESET_ID;
  const savedPrompt = String(settings[promptKey] || '').trim();
  const cleanedPrompt = isLegacyBuiltinPrompt(savedPrompt) ? '' : savedPrompt;

  let label = '미설정';
  if (presetId === CUSTOM_PRESET_ID) label = cleanedPrompt ? '직접 입력' : '미설정';
  ...
  const configured =
    ...
    (presetId === learnedId ||
      ...
      !!findFn(presetId, settings[customPresetsKey] || []));
...
function buildLearnedStyleSettingsPatch(settings = {}, context = 'review', response = {}, extra = {}) {
  ...
  const modeKey = isInquiry ? 'inquiryStyleActiveMode' : 'styleActiveMode';

  const learned = {
    id: learnedId,
    name: isInquiry
      ? `응대 지침서 (${response.sampleCount || 0}건)`
      : `내 스타일 (샘플 ${response.sampleCount || 0}개)`,
  ...
  const mergedFlow = extra[flowKey] || response.sampleFlow || settings[flowKey] || {};
  let activeMode = 'pick';
  if (mergedFlow.source === 'paste' || mergedFlow.source === 'file') activeMode = 'paste';
...
function showStyleApplyConfirm(options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const meta =
      sampleCount > 0
        ? `<div style="font-size:13px;color:#666;margin-bottom:16px;">예전 답글 ${sampleCount}개를 참고해 새 말투를 만들었어요.</div>`
        : '';
    ...
    const currentBlock = showCurrent
      ? `<div style="margin-bottom:14px;">
        ...
        <pre class="ss-style-preview ss-style-preview-current" style="margin:0;padding:12px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;font-size:12px;line-height:1.55;white-space:pre-wrap;word-break:break-word;max-height:140px;overflow:auto;"></pre>
      </div>`
      : '';

    const keepLabel = showCurrent ? '지금 말투 유지' : '취소';
    ...
    card.innerHTML = `
      ...
      <div style="margin-bottom:20px;">
        <div style="font-size:13px;font-weight:700;color:#1d4ed8;margin-bottom:6px;">새 말투 지침</div>
        <div style="font-size:12px;color:#64748b;margin-bottom:8px;">비슷한 의미끼리 묶여 있어요. 수정·삭제하거나 새 지침을 추가한 뒤 저장하세요.</div>
        <div class="ss-style-guideline-host"></div>
        <div class="ss-style-confirm-error" hidden style="margin-top:8px;font-size:12px;color:#b91c1c;">지침을 1개 이상 남겨 주세요.</div>
      </div>
      <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap;">
        ...
        <button type="button" data-action="apply" style="padding:12px 18px;border:none;border-radius:10px;background:#2563eb;color:#fff;font-weight:700;cursor:pointer;">이 말투 저장</button>
      </div>
    `;
    ...
    const host = card.querySelector('.ss-style-guideline-host');
    const errorEl = card.querySelector('.ss-style-confirm-error');
    const editor =
      typeof mountGuidelineItemsEditor === 'function'
        ? mountGuidelineItemsEditor(host, {
            text: newPrompt,
    ...
    if (!editor) {
      ...
      fallback.className = 'ss-style-preview-new';
      fallback.rows = 12;
      ...
      fallback.value = String(newPrompt || '').trim();
    ...
    function tryApply() {
      const editedPrompt = editor
        ? editor.getSerialized()
        : String(host.querySelector('textarea')?.value || '').trim();
      if (!editedPrompt) {
...
async function confirmAndApplyLearnedStyle(settings, context, response, extraSettingsPatch = {}) {
  ...
  const choice = await showStyleApplyConfirm({
    title: snapshot.configured ? '새 말투로 바꿀까요?' : '만든 말투 확인',
    subtitle: snapshot.configured
      ? '지금 쓰는 말투가 있어요. 아래 지침을 고친 뒤 저장할지 선택하세요.'
      : 'AI가 만든 말투입니다. 마음에 들면 저장, 고치고 싶으면 항목별로 수정하세요.',
