...
function createStyleChannel(config) {
  ...
  let customPresets = [];
  let tonePresetId = CUSTOM_PRESET_ID;
  let activeStyleMode = 'pick';
  ...
  function syncLearnedPromptEditor() {
    ...
    if (els.learnedSystemPrompt) {
      if (els.learnedSystemPrompt.value !== text) els.learnedSystemPrompt.value = text;
      ensureGuidelineSummaryCard(els.learnedSystemPrompt);
    }
    if (els.learnedSystemPromptPaste) {
      if (els.learnedSystemPromptPaste.value !== text) els.learnedSystemPromptPaste.value = text;
      ensureGuidelineSummaryCard(els.learnedSystemPromptPaste);
    }
  }

  function countGuidelineItems(text) {
    if (typeof parseGuidelineItems === 'function') {
      return parseGuidelineItems(text).length;
    }
    return String(text || '')
      .split(/\n+/)
      .map((line) => line.trim())
  ...
  function previewGuidelineText(text, max = 160) {
    const raw = String(text || '')
      ...
      .slice(0, 2)
      .join(' · ');
    if (!raw) return '저장된 지침이 없습니다.';
    return raw.length > max ? `${raw.slice(0, max)}…` : raw;
  }

  function ensureGuidelineSummaryCard(textarea) {
    if (!textarea) return;
    let card = textarea.nextElementSibling;
    const bindManage = (host) => {
      const btn = host.querySelector('.guideline-manage-btn');
      if (!btn || btn.dataset.bound === '1') return;
      btn.dataset.bound = '1';
      btn.addEventListener('click', (event) => {
        event.preventDefault();
        event.stopPropagation();
        openGuidelineEditorWindow(channelId);
      });
    };

    if (!card || !card.classList.contains('guideline-summary-card')) {
      textarea.hidden = true;
      textarea.setAttribute('aria-hidden', 'true');
      card = document.createElement('div');
      card.className = 'guideline-summary-card';
      textarea.insertAdjacentElement('afterend', card);
    }

    if (!card.querySelector('.guideline-summary-top')) {
      card.innerHTML = `
        <div class="guideline-summary-top">
          <div class="guideline-summary-count"></div>
          <div class="guideline-summary-state">적용 중</div>
        </div>
        <button type="button" class="guideline-manage-btn">지침 관리</button>
      `;
    }

    bindManage(card);
    const count = countGuidelineItems(textarea.value);
    const countEl = card.querySelector('.guideline-summary-count');
    const stateEl = card.querySelector('.guideline-summary-state');
    if (countEl) countEl.textContent = count ? `${count}개 지침` : '지침 없음';
    if (stateEl) stateEl.hidden = !count;
  }

  function openGuidelineEditorWindow(contextId) {
    const url =
      `${chrome.runtime.getURL('guideline-editor.html')}?context=${encodeURIComponent(
        contextId || channelId
      )}`;
    ...
    const ready =
      mode === 'pick'
        ...
        : mode === 'paste'
          ? isPasteConfigured()
          : mode === 'preset'
          ? !!findChannelPreset(tonePresetId)?.prompt
          : isCustomConfigured();
  ...
  function updateStyleModeCardsUI() {
    ...
    const configured =
      isPickConfigured() || isPasteConfigured() || isCustomConfigured() || hasLearnedPreset();
    const panel = list.closest('.mode-panel');
    if (panel) panel.classList.toggle('is-style-configured', configured);
    list.classList.toggle('is-configured', configured);
    ...
    preservePanelScroll(() => {
      list.querySelectorAll('.style-mode-card').forEach((card) => {
        ...
        const isActive = activeStyleMode === mode;
        const appliedNow =
          (mode === 'pick' && isPickConfigured()) ||
          (mode === 'paste' && isPasteConfigured()) ||
          (mode === 'custom' && isCustomConfigured());
        card.classList.toggle('active', isActive);
        card.classList.toggle('is-applied', isActive && appliedNow);
    ...
    preservePanelScroll(() => {
      ...
      if (mode === 'pick') {
        ...
        if (pid !== CUSTOM_PRESET_ID && pid !== learnedPresetId) {
          ...
          applyPresetSelection(CUSTOM_PRESET_ID, savedCustomPrompt || '', save, {
            skipModeSync: true,
            skipPanelUi: true,
          });
        }
  ...
  function initFromSettings(settings) {
    ...
    tonePresetId = settings[storageKeys.tonePresetId] || CUSTOM_PRESET_ID;
    savedCustomPrompt = settings[storageKeys.savedCustomPrompt] || '';
    savedPresetId = settings[storageKeys.savedPresetId] || '';
    if (
      savedPresetId === learnedPresetId ||
      savedPresetId === CUSTOM_PRESET_ID ||
      isLegacyBuiltinPresetId(savedPresetId)
    ) {
      savedPresetId = '';
    }
    sampleFlow = {
    ...
    const savedPrompt = String(settings[storageKeys.systemPrompt] || '').trim();
    const isLegacyBuiltin =
      isLegacyBuiltinPresetId(tonePresetId) || isLegacyBuiltinPrompt(savedPrompt);
    const hasLearned = !!findChannelPreset(learnedPresetId)?.prompt;
    const hasCustomPrompt =
      tonePresetId === CUSTOM_PRESET_ID && savedPrompt && !isLegacyBuiltinPrompt(savedPrompt);
    const hasUserPreset =
      !!findChannelPreset(tonePresetId)?.prompt &&
      tonePresetId !== learnedPresetId &&
      !isLegacyBuiltinPresetId(tonePresetId);

    if (isLegacyBuiltin && !hasLearned && !hasCustomPrompt && !hasUserPreset) {
      tonePresetId = CUSTOM_PRESET_ID;
      savedCustomPrompt = '';
      if (els.systemPrompt) els.systemPrompt.value = '';
      applyPresetSelection(CUSTOM_PRESET_ID, '', false, { skipModeSync: true });
      activeStyleMode = 'pick';
      onSettingsDirty();
    } else if (savedPrompt && !isLegacyBuiltin) {
      applyPresetSelection(tonePresetId, savedPrompt, false, { skipModeSync: true });
      activeStyleMode = inferActiveStyleMode(settings);
    } else if (hasLearned) {
      applyPresetSelection(learnedPresetId, null, false, { skipModeSync: true });
      activeStyleMode = inferActiveStyleMode(settings);
    } else {
      applyPresetSelection(CUSTOM_PRESET_ID, '', false, { skipModeSync: true });
      activeStyleMode = settings[storageKeys.activeStyleMode] || 'pick';
    }

    if (!settings[storageKeys.activeStyleMode] && activeStyleMode === 'preset') {
      activeStyleMode = 'pick';
    } else if (settings[storageKeys.activeStyleMode]) {
      activeStyleMode = inferActiveStyleMode(settings);
    }
  ...
  function renderPresetOptions() {
    ...
    select.value =
      findChannelPreset(tonePresetId) || tonePresetId === CUSTOM_PRESET_ID
        ? tonePresetId
        : CUSTOM_PRESET_ID;
  }
  ...
  function getActiveStyleBannerContent() {
    ...
    else if (findChannelPreset(tonePresetId)) variant = 'preset';

    return { variant, prompt: prompt || '아직 지침이 없습니다. 기존 답글로 만들거나 직접 작성해 주세요.', method };
  }
  ...
  function getActiveStyleMethodLabel() {
    ...
    return preset ? `내 스타일 · ${preset.name}` : '지침 미설정';
  }
  ...
  function updateSampleFlowUI() {
    ...
    if (features.stylePick && els.openStylePickBtn) {
      ...
      if (isAnalyzed && isApplied && sampleFlow.source === 'seller-pick') {
        els.openStylePickBtn.textContent = '다시 만들기';
      } else {
        els.openStylePickBtn.textContent =
          channelId === 'inquiry' ? '기존 답변으로 응대 지침서 만들기' : '기존 답글로 내 말투 만들기';
      }
    }

    if (isAnalyzed && isApplied) {
      if (els.sampleFlowStatus) {
        els.sampleFlowStatus.textContent = '';
        els.sampleFlowStatus.className = 'sample-flow-status';
        els.sampleFlowStatus.hidden = true;
      }
      if (els.analyzeBtn && (sampleFlow.source === 'paste' || sampleFlow.source === 'file')) {
        els.analyzeBtn.disabled = false;
        els.analyzeBtn.classList.remove('loading');
        els.analyzeBtn.classList.add('done');
        els.analyzeBtn.textContent = '다시 만들기';
      }
      updateStyleModeCardsUI();
      return;
    }

    if (els.sampleFlowStatus) els.sampleFlowStatus.hidden = false;
