...
function createStyleChannel(config) {
  ...
  function onSampleRepliesInput() {
    ...
    if (count >= 2) {
      if (!sampleFlow.loadedAt || sampleFlow.source === 'paste') {
        ...
        sampleFlow.loadedCount = count;
        invalidateSampleAnalysis('예시가 바뀌었어요. 다시 「말투 만들기」를 눌러 주세요.');
      }
    } else {
  ...
  function updateSampleFlowUI() {
    ...
    if (features.stylePick && els.openStylePickBtn) {
      ...
      els.openStylePickBtn.textContent = '판매자센터에서 내 답글 고르기';
    ...
    if (isAnalyzingSamples || sampleFlow.analyzing) {
      if (els.analyzeBtn) {
        ...
        els.analyzeBtn.textContent = '만드는 중...';
      }
    ...
    if (els.analyzeBtn) {
      ...
      if (isAnalyzed) {
        els.analyzeBtn.textContent = `✓ 만들기 완료 (${sampleFlow.analyzedCount}개) · 다시 만들기`;
        els.analyzeBtn.classList.add('done');
      } else {
        els.analyzeBtn.textContent = '말투 만들기';
    ...
    if (isAnalyzed && isApplied) {
      setSampleFlowStatus(
        `✓ 완료 · ${sampleFlow.sourceLabel || '예시'} ${count}개\n지금부터 AI가 이 말투로 ${label} 답글을 씁니다.`,
        'success'
      );
      return;
    }
    ...
    if (isAnalyzed) {
      setSampleFlowStatus(
        `✓ 말투 만들기 완료 · ${sampleFlow.analyzedCount}개\n위에서 「내 스타일」을 선택하면 적용됩니다.`,
        'success'
      );
      return;
    }
    ...
    setSampleFlowStatus(
      features.stylePick
        ? `${label} 예전 답글 2개 이상을 넣거나, 판매자센터에서 고른 뒤 「말투 만들기」를 누르세요.`
        : `${label} 예전 답글 2개 이상을 넣은 뒤 「말투 만들기」를 누르세요.`
    );
  }
  ...
  async function onAnalyzeSamples() {
    ...
    if (!(await hasAiCredentialsAsync(apiKey))) {
      setSampleFlowStatus('[설정]에서 로그인하거나 API 키를 입력해 주세요.', 'warn');
      return;
    }
    ...
    try {
      ...
      setSampleFlowStatus(
        `✓ 새 스타일 적용 · 샘플 ${response.sampleCount}개로 「내 스타일」이 저장되었습니다.\n${label} AI 답변 생성 시 이 톤이 적용됩니다.`,
        'success'
      );
      updateSampleFlowUI();
    } catch (err) {
